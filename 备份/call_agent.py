"""
上课时段 AI 电话代接系统 — 多 Agent 协作版
支持：语音识别 → RAG规则匹配 → 风险/意图多Agent分析 → 智能回复 → TTS语音播放
"""
import whisper
import edge_tts
import asyncio
...
class RAGRetriever:
    ...
    def _ngram_match_score(self, call_text: str, doc_text: str) -> float:
        """多粒度 n-gram 加权 Jaccard 重叠分（0~1）。
        用 max(len(c), len(d)) 做分母偏向召回——口语和书面语长度差异大。"""
        total = 0.0
    ...
    def search(self, call_text: str) -> str:
        ...
        # --- 第1步：n-gram 倒排粗筛（选候选文档） ---
        candidate_scores = {}  # doc_idx -> accumulated ngram hit count
...
class DeepSceneAnalyzer:
    """
    深度场景分析Agent：在风险判断前，先对来电做多维度分析
    提取：场景类别、紧急程度、话术模式、语义锚点、矛盾信号
    """
...
"""
    ...
    def _fix_reply(call_text: str, reply: str) -> str:
        """关键词兜底：修正 LLM 没按模板生成的回复。"""
        # 外卖柜故障/打不开/坏了 → 不能回复"换套餐"
...
async def main():
    print("\n" + "=" * 60)
    print(" AI 电话代接系统 — 音频文件模式（循环测试）")
    print(" 选择音频文件后自动处理，取消选择则退出")
    print("=" * 60)

    # 系统只初始化一次
    asr, rag, scheduler, tts = init_system()
    round_num = 0
    ...
    try:
        while True:
            ...
            audio_path = filedialog.askopenfilename(
                title=f"选择来电语音文件（第 {round_num} 轮，取消则退出）",
            ...
            print(f"\n{'=' * 60}")
            print(f" 第 {round_num} 轮 — 已选择：{os.path.basename(audio_path)}")
            print(f"{'=' * 60}")

            try:
                # 0. 播放原始语音（非阻塞） + ASR 识别（线程并行）
                print("\n 【播放来电】正在播放原始语音…")
                ...
                try:
                    ...
                    print(f" 播放原始语音失败（不影响后续处理）：{e}")

                # ASR 在后台线程中运行，与音频播放并行
                transcribe_task = loop.run_in_executor(None, asr.transcribe, audio_path)
                ...
                # 等待 ASR 识别完成
                call_text = await transcribe_task
